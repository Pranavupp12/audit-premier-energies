...
function normalizeMultiEmailField(val) {
  ...
  try {
    ...
    const issues = rows.map((r) => {
      ...
      return {
        ...r,
        evidenceReceived: ev,
        personResponsible: toArray(r.personResponsible),
        approver: toArray(r.approver),
        cxoResponsible: toArray(r.cxoResponsible),
        coOwner: toArray(r.coOwner),
      };
    });
...
async function sendToMany(toStrOrArr, subject, htmlContent) {
  // Convert comma-separated string or array to an array of emails:
  const emails = Array.isArray(toStrOrArr)
    ? toStrOrArr
    : (toStrOrArr || "")
        .split(",")
        .map((e) => e.trim())
  ...
  // Send to each non-empty, valid-looking string:
  for (const email of emails) {
    // You might validate more strictly if desired
    if (email) await sendEmail(email, subject, htmlContent);
  }
}
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // 5) Notifications (send to all emails for each role)
      ...
      await Promise.all([
        sendToMany(row.personResponsible, "Proof Submitted", htmlForPerson),
        sendToMany(
          row.cxoResponsible,
          `${uploadedBy} submitted proof`,
          htmlForCXO
        ),
        sendToMany(row.approver, "New Proof Uploaded", htmlForAuditor),
      ]);
