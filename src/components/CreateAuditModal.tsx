  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await Promise.all(formData.entities.map(async ent => {
        ...
        // personResponsible
        if (samePerson) {
          formData.personResponsible.forEach(pr => {
            if (pr) payload.append('personResponsible', pr);
          });
        } else {
          // unchanged for per-entity entry
          const pr = formData.byEntityResponsibles[ent] || '';
          if (pr) payload.append('personResponsible', pr);
        }
